...
test.beforeAll(async ({ request }) => {
  ...
  const emptyCategoryResponse = await request.post("/api/workspace-categories", {
    data: { name: `Drilldown Empty ${crypto.randomUUID()}` },
  });
  expect(emptyCategoryResponse.status(), await emptyCategoryResponse.text()).toBe(201);
  emptyCategoryId = (await emptyCategoryResponse.json()).id;
...
test.afterAll(async ({ request }) => {
  ...
  expect((await request.delete(`/api/workspace-categories/${emptyCategoryId}`)).status()).toBe(200);
});
...
test("month and mode preserve empty slices; allocations expose full source details", async ({ page }) => {
  const list = page.getByRole("region", { name: /Included line items/ });
  const sharedHousingRow = list.getByRole("row").filter({ hasText: "Drilldown shared housing" });
  await page.goto(`/reports?view=month&month=${month}&mode=payment_date&kind=expense&category=${emptyCategoryId}#line-items`);
  ...
  await expect(sharedHousingRow).toContainText("Full source amount: 600.00 ILS");
